...
deleteMacroBtn.addEventListener("click", (e) => {
  let listOfMacros = Array.from(
    sidebarList.querySelectorAll(".macro-list-item")
  );
...
function updateMacroString(key) {
  ...
  let sidebarItem = sidebarList.querySelector(
    "label[for=cb-keybind-" + key + "]"
  );
  if (sidebarItem) {
...
function toggleMacroMode(button) {
  if (button.classList.toggle("check")) {
    // returns true if 'check' is added to classList
...
function updateStopwatches() {
  ...
  let going = stopwatchArray.filter(
    (sw) => sw.timeButton && sw.timeButton.classList.contains("going")
  );
  for (let i = 0; i < going.length; i++) {
    //update each stopwatch in array
    let dur = (new Date() - going[i].startTime) / 1000;
    going[i].timeButton.textContent = parseFloat(
      Math.round((+going[i].prevTime + dur) * 100) / 100
...
function keyDownEvent(e) {
  if (e.target.nodeName !== "INPUT") {
    //do nothing if currently in a textbox
...
function keybindChangeEvent(e) {
  if (e.target.value !== "") {
    ...
    keybindObj[e.target.value] = [
      stopwatchArray.find((sw) => getParentStopwatch(e.target).id === sw.id),
    ];
  } else {
...
function checkKeybindAvailable(kb) {
  ...
  if (swArray && swArray.length > 0) {
    if (
      confirm(
        "The stopwatch(es) " +
          swArray.map((sw) => sw.name).join(", ") +
          " already has this keybind. Replace?"
      )
    ) {
      removeKeybind(kb);
    } else {
...
function nameChangeEvent(e) {
  let stopwatchChanged = stopwatchArray.find(
    (sw) => getParentStopwatch(e.target).id === sw.id
  );
  if (stopwatchChanged) {
